...
export class PerformerService {
    ...
    async create(performer: PerformerEntity): Promise<PerformerEntity> {
        if(performer.description.trim().length > 100) throw new BusinessLogicException('The performer description must be less than 100 characters.', BusinessError.BAD_REQUEST);
